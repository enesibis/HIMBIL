...
LAN mode (`client-flutter/lib/game/lan/lan_host_session.dart`) is a **third** authoritative wrapper, shaped like `gameSession.ts` (roster/`chooseCard(playerId,...)`/`view(playerId)`, never leaks another player's hand) but built directly on the *already player-index-agnostic* `rules.dart` pure functions and on `bot_ai.dart` for takeover-bot behavior — it doesn't re-implement rules or bot heuristics, only the room-glue layer (`lan_host_server.dart`, mirroring `HimbilRoom.ts`'s timer/broadcast responsibilities over raw TCP instead of Colyseus).

### Server-authoritative design (live in online games)
The architecture from kılavuz §3 is now real for online play: clients send intent only (`chooseCard`, `slamPress`), `server/rooms/gameSession.ts` runs the authoritative loop, and `HimbilRoom` pushes each player a filtered view that never contains another player's hand. Countdown timing is sent as absolute deadlines (epoch ms), never remaining time. Offline bot games still compute locally in `GameController` by design — that mode has no server to defer to and must keep working without one. LAN mode (see above) applies the same server-authoritative shape without a server: whichever phone taps "Oda Aç" runs `LanHostSession`/`LanHostServer` locally and is authoritative for that match; if that phone's app closes, the match ends (no host migration, same limitation as if the Colyseus process died).
