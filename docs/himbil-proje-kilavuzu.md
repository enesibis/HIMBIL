...
3. ✅ **Otoriter sunucu + 1 gerçek client.** Kural motorunu sunucuya taşı; client intent yollar, sunucu state push eder. İki instance lokal test. *(Tamamlandı: sunucu `HimbilRoom` + `gameSession.ts` otoriter döngüyü çalıştırıyor; client ekranları `GameDriver` soyutlaması üzerinden sunucu state'ini tüketiyor (`ServerGameDriver`), yerel bot modu offline/fallback olarak kaldı. 4 istemcili uçtan uca test `client-flutter/tool/e2e_online_match.dart` ile.)*
4. ✅ **Odalar + matchmaking (oda kodu).** Çok oyuncu, kodla katılma. *(Tamamlandı: lobi gerçek matchmaking'e bağlı — oda kur sunucunun verdiği kodu gösterir, kodla katıl `filterBy` ile eşleşir, hızlı oyna joinOrCreate; oda dolunca maç otomatik başlar. Sunucu ulaşılamazsa bot lobisine düşülür.)*
5. ✅ **Gerçek zamanlı takas loop'u + slam yarışı.** Tick, eşzamanlı takas, slam çözümü, yanlış-slam cezası. *(Tamamlandı ve uçtan uca oynanıyor: 4 sn takas tick'i, 4 sn slam penceresi (herkes basınca erken kapanır), turlar arası 4 sn scoring molası, deadline'lar mutlak epoch ms olarak gönderilir. His/zamanlama ayarı gerçek oyuncularla test edildikçe iterasyona açık.)*
6. ✅ **Dayanıklılık.** Yeniden bağlanma, kopma yönetimi, gecikme toleransı. *(Tamamlandı: 30 sn reconnect grace + client'ta otomatik yeniden bağlanma + oyun ekranında bağlantı şeridi; grace dolarsa koltuk sunucu botuna devredilir (`server/rooms/botPlayer.ts` — client bot sezgisiyle aynı) ve maç akmaya devam eder.)*
7. 🔶 **Kalıcılık & meta.** Hesap, istatistik, leaderboard, kozmetikler. *(Büyük ölçüde: misafir hesaplar online katılımda odaya bağlanıyor, maç sonu ödülleri sunucu defterine yazılıyor, `GET /leaderboard` bu defterden hilelenemez bir sıralama üretiyor ve Profil sekmesi gösteriyor; analytics olayları da sunucuda toplanıyor. Kalan: jeton/envanter ekonomisinin cihazdan sunucuya tam taşınması (IAP öncesi şart) ve gerçek hesap bağlama (Google/Apple).)*
...
Aşama 1-6 tamamlandı: oyun online olarak uçtan uca oynanıyor (lobi → matchmaking → otoriter takas/slam döngüsü → tur/maç sonu → reconnect + bot devralma), offline bot modu her zaman çalışan taban olarak duruyor. Aşama 7'nin de büyük kısmı yerinde (misafir hesap defteri, sunucu ödülleri, liderlik tablosu, analytics). **Sıradaki somut adım: ekonomiyi sunucuya tam taşımak** — `PlayerSession`'daki jeton bakiyesi ve kozmetik envanterini misafir-hesap API'sine geçir (mağaza satın alımı sunucuda düşülsün, client `GET /guest/me`'yi yansıtsın); bu, IAP eklemenin ön şartı. Ardından gerçek hesap bağlama (Google/Apple) ve gerçek cihaz/oyuncularla his/zamanlama testi. Aşama 8 (Redis, çok-node, yük testi) tek sunucu yetmemeye başlayana kadar bilinçli olarak erteli (§8: erken optimizasyon).
